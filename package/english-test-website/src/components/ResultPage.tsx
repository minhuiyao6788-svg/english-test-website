...
import { Trophy, Clock, Target, BookOpen, RotateCcw, Home, Star, Award } from 'lucide-react';
  ...
  const calculateResults = () => {
    ...
    const results = questions.map((question, index) => {
      ...
      return {
        ...
        userAnswer: userAnswer ? String.fromCharCode(65 + parseInt(userAnswer)) : undefined,
        correctAnswer: String.fromCharCode(65 + parseInt(question.correct_answer)),
        isCorrect,
    ...
    if (testType === 'vocabulary') {
      ...
      if (score >= 90) {
        level = '高级';
        levelDescription = '您的托福水平已达到高级标准，可以申请顶尖大学';
        recommendations = ['继续保持高强度学习', '关注学术英语能力', '参与英语学术讨论'];
      } else if (score >= 80) {
        level = '中高级';
        levelDescription = '您的托福水平良好，可以申请大部分大学';
        recommendations = ['加强学术写作', '提升听力理解', '练习口语流利度'];
      } else if (score >= 70) {
        level = '中级';
        levelDescription = '您的托福水平中等，建议进一步提升';
        recommendations = ['扩大学术词汇', '加强阅读速度', '练习考试技巧'];
      } else {
        level = '初级';
        levelDescription = '您的托福水平需要加强，建议系统学习';
        recommendations = ['从基础语法开始', '大量练习听力', '熟悉考试格式'];
      }
